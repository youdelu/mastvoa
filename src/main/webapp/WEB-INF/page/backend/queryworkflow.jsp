...
<div class="container">
 ...
 <div class="row">
  <div class="col-md-6">
	<form action="queryworkflow.do" method="post" class="form-horizontal" role="form">
				...
				<table class="table table-striped">
					...
					<tbody>
						...
						<tr <%=row.get(4).toString().equals("2") ? " class=\"danger\"" : ""%>>
							...
							<td><a
								href="qdetailworkflow.do?flowid=<%=row.get(0).toString()%>"><span
			...
			<div class="text-center">
				<ul class="pagination">
					...
					<%
						if (pageNumber > 1) {
					%>
					<li><a href="queryworkflow.do?year=<%=request.getAttribute("year") %>&month=<%=request.getAttribute("month") %>&tense=<%=request.getAttribute("tense") %>&status=<%=request.getAttribute("status")%>&querytype=<%=request.getAttribute("querytype")%>&queryid=<%=request.getAttribute("queryid")%>&pageNumber=<%=pageNumber - 1%>">&laquo;</a></li>
					<%
						} else {
					%>
					...
					<li <%=(i == pageNumber) ? " class=\"active\"" : ""%>><a
						href="queryworkflow.do?year=<%=request.getAttribute("year") %>&month=<%=request.getAttribute("month") %>&tense=<%=request.getAttribute("tense") %>&status=<%=request.getAttribute("status")%>&querytype=<%=request.getAttribute("querytype")%>&queryid=<%=request.getAttribute("queryid")%>&pageNumber=<%=i%>"><%=i%></a></li>
					<%
						}
					...
					%>
					<li><a href="queryworkflow.do?year=<%=request.getAttribute("year") %>&month=<%=request.getAttribute("month") %>&tense=<%=request.getAttribute("tense") %>&status=<%=request.getAttribute("status")%>&querytype=<%=request.getAttribute("querytype")%>&queryid=<%=request.getAttribute("queryid")%>&pageNumber=<%=pageNumber + 1%>">&raquo;</a></li>
					<%
						} else {
					%>
