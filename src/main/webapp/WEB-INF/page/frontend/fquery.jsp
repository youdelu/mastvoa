...
<div class="container">
	...
	<form action="fquery.do" method="post" class="form-horizontal" role="form">
				...
				<table class="table table-striped">
					...
					<tbody>
						...
						<tr <%=row.get(4).toString().equals("2") ? " class=\"danger\"" : ""%>>
							...
							<td><a
								href="<%=request.getAttribute("mytype") %>.do?flowid=<%=row.get(0).toString()%>"><span
			...
			<div class="text-center">
				<ul class="pagination">
					...
					<%
						if (pageNumber > 1) {
					%>
					<li><a href="fquery.do?mytype=<%=request.getAttribute("pType") %>&year=<%=request.getAttribute("myYear") %>&month=<%=request.getAttribute("myMonth") %>&pageNumber=<%=pageNumber - 1%>">&laquo;</a></li>
					<%
						} else {
					%>
					...
					<li <%=(i == pageNumber) ? " class=\"active\"" : ""%>><a
						href="fquery.do?mytype=<%=request.getAttribute("pType") %>&year=<%=request.getAttribute("myYear") %>&month=<%=request.getAttribute("myMonth") %>&pageNumber=<%=i%>"><%=i%></a></li>
					<%
						}
					...
					%>
					<li><a href="fquery.do?mytype=<%=request.getAttribute("pType") %>&year=<%=request.getAttribute("myYear") %>&month=<%=request.getAttribute("myMonth") %>&pageNumber=<%=pageNumber + 1%>">&raquo;</a></li>
					<%
						} else {
					%>
...
<%} %>
	<%@ include file="../jspf/frontend/footer.jsp"%>